...
const ContactForm = () => {
  ...
  const handleChange = (e, input) => {
    setContacto({ ...contacto, [input]: e.target.value.trim() });
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (contacto.nombre === '' || contacto.email === '') {
      ...
      setError({ error: true, mensaje: 'El email es inválido' });
      return;
    } else {
  ...
  return (
    <div className="form-box">
      {error.error && (
        <p className="error">
          Por favor, verifica que la información sea correcta: <br />
          {error.mensaje}
        </p>
      )}
      {isSended && (
        <>
          ...
          <p>Te contactaremos a la brevedad posible vía email</p>
        </>
      )}
      {!isSended && (
